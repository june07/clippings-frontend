<template>
	<v-container class="h-100 d-flex align-center justify-center flex-column" fluid>
		...
		<v-card rounded="xl" class="pa-4" :width="smAndDown ? '-webkit-fill-available' : '800px'" elevation="0" v-if="location.pathname === '/home'">
			...
			<div class="d-flex justify-center">
				<ul class="mt-4 font-weight-bold text-body-2" :class="smAndDown ? 'ml-1' : ''">
					...
					<li>
						100% open source (Vue.js <a href="https://github.com/june07/clippings-frontend" rel="noopener" target="_blank">frontend</a>, Node.js <a href="https://github.com/june07/clippings-backend" rel="noopener" target="_blank">backend</a>, and
					...
					<v-row class="d-flex align-center">
						...
						<v-col :cols="9" class="text-start py-0 pl-0">
							...
							<a v-else style="text-decoration: none" :href="getWebURL(listingPid)">
								<div class="text-caption text-truncate text-green"><v-icon icon="link" class="mr-2" />{{ getWebURL(listingPid) }}</div>
							</a>
						</v-col>
						<v-spacer />
					</v-row>
					<v-row class="d-flex align-center">
						...
						<v-col :cols="9" class="text-start py-0 pl-0">
							...
							<a v-else style="text-decoration: none" :color="archiveWaitingToBeReady ? 'grey-lighten-2' : 'green'" :href="getArchiveURL(listingPid)" target="_blank">
								<div class="text-caption text-truncate" :class="archiveWaitingToBeReady ? 'text-grey-lighten-2' : 'text-green'"><v-icon icon="link" class="mr-2" />{{ getArchiveURL(listingPid) }}</div>
							</a>
						</v-col>
						<v-spacer />
					</v-row>
					<v-row class="d-flex align-center">
						...
						<v-col :cols="9" class="text-start py-0 pl-0">
							...
							<a v-else style="text-decoration: none" :href="getCodeURL(listingPid)" target="_blank">
								<div class="text-caption text-truncate text-green"><v-icon icon="link" class="mr-2" />{{ getCodeURL(listingPid) }}</div>
							</a>
						</v-col>
						<v-spacer />
					</v-row>
		...
		<emergency-setup-dialog
			v-model="dialogs.emergencySetup"
			@close="dialogs.emergencySetup = false"
			...
			@create:alert="createAlert => emergencySetupHandler({ createAlert })"
			@update:alert="updateAlert => emergencySetupHandler({ updateAlert })"
...
const rules = {
	url: [v => !!v || `url is required.`, v => (v && /https:\/\/.*\.craigslist\.org\/.+/.test(v)) || /\d{10}/.test(v) || `url is not valid or not supported`],
}
