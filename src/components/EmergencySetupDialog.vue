        ...
        <v-card rounded="xl" class="pa-4" style="opacity: 0.96">
            ...
            <v-card-text>
                ...
                <div v-if="store.settings.emergencyContact.contacts.length">
                    ...
                    <v-list lines="one">
                        <v-list-item v-for="listContact of store.settings.emergencyContact.contacts" :key="listContact._id" :title="listContact.name" :subtitle="listContact.email" @click="setActiveContactHandler(listContact)" :class="listItemClass('contact', listContact._id)" :style="!success && justLoaded ? 'transition: background-color 3s' : ''">
                ...
                <v-expansion-panels v-model="expansionPanel" multiple>
                    ...
                    <div class="w-100" v-if="userMessages.length">
                        ...
                        <v-list lines="one">
                            <v-list-item v-for="(listMessage, index) of userMessages" :key="listMessage._id" :title="listMessage.title" :subtitle="listMessage.text" @click="setActiveMessageHandler(listMessage)" :class="listItemClass('message', listMessage._id)" :style="!success && justLoaded ? 'transition: background-color 3s' : ''">
                                <template v-slot:prepend>
...
const rules = {
    ...
    title: [
        (v) => !v || v && v.length < 100 || `Title is too long.`,
    ],
    relationship: [
